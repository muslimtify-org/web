...
```bash
curl -LO https://github.com/muslimtify-org/libmuslim/releases/download/2026.08.22/prayertimes.h
```
...
The tag is a calendar date. Each header carries its own semantic version in its banner, and they move independently, so `2026.08.22` contains `prayertimes.h` `v0.2.3`, `hijri.h` `v0.1.1` and `timezone.h` `v0.1.1`, while the `2026.08.21` tag before it carried `hijri.h` at `v0.1.0`. That is expected, not a mismatch. Pin the tag to fetch a file, read the header version to reason about compatibility.
:::
